...
exports.update = function(req,res,next){
	db.query("UPDATE degree SET degree_code=?, description=?, total_units=? WHERE degree_id=?", 
		[req.body.degree_code,req.body.description,req.body.total_units,req.body.degree_id], function(err,rows){
		if(err) return next(err);
		res.send(rows);
	});
};
